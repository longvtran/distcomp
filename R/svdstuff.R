...
############ LONG: Add protobuf option to SVDMaster ################
SVDMaster <- R6Class("SVDMaster",
                     private = list(
                       ...
                       mapFn =  function(site, arg, method) {
                         ...
                         q <- if (sType == "json") {
                           POST(.makeOpencpuURL(urlPrefix=site$url, fn="executeMethod",
                                                serialize_type = "json"),
                                body = toJSON(payload),
                     ...
                     public = list(
                       ...
                       run = function(k = private$k, thr = 1e-8, max.iter = 100) {
                         ...
                         ############ LONG: Add protobuf option to sitesOK ################
                         sitesOK <- sapply(sites,
                                           function(x) {
                                             ...
                                             sType <- getConfig()$serialize_type
                                             q <- if (sType == "json") {
                                               POST(url = .makeOpencpuURL(urlPrefix=x$url, fn="createInstanceObject", 
                                                                          serialize_type = "json"),
                                                    body = toJSON(payload),
                                                    add_headers("Content-Type" = "application/json"),
                                                    config=getConfig()$sslConfig
                         ...
                         ############ LONG: Add protobuf option to pVals ################
                         pVals <- sapply(sites,
                                         function(x) {
                                           payload <- list(objectId = x$instanceId, method = "getP")
                                           sType <- getConfig()$serialize_type
                                           q <- if (sType == "json") {
                                             POST(.makeOpencpuURL(urlPrefix=x$url, fn="executeMethod",
                                                                  serialize_type = "json"),
                                                  body = toJSON(payload),
                                                  add_headers("Content-Type" = "application/json"),
                                                  config=getConfig()$sslConfig
                         ...
                         sitesOK <- sapply(sites,
                                           function(x) {
                                             ...
                                             sType <- getConfig()$serialize_type
                                             q <- if(sType == "json") {
                                               POST(url = .makeOpencpuURL(urlPrefix=x$url, fn="destroyInstanceObject",
                                                                          serialize_type = "json"),
                                                    body = toJSON(payload),
                                                    add_headers("Content-Type" = "application/json"),
                                                    config=getConfig()$sslConfig
